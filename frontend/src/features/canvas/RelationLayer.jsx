import { endpointLabelPosition, selfLoopGeometry } from "../../relation-geometry.js";
...
export function RelationLayer({ diagram, classDimensions = {}, selectedId, hoveredId, onSelect, onHover }) {
  ...
  return <svg className="relations" aria-label="Relaciones"><defs>
    <marker id="uml-composition-filled" markerWidth="18" markerHeight="18" refX="16" refY="9" orient="auto-start-reverse" markerUnits="userSpaceOnUse"><path className="marker-composition" d="M0 9 L9 0 L18 9 L9 18 Z" /></marker>
    <marker id="uml-aggregation-hollow" markerWidth="18" markerHeight="18" refX="16" refY="9" orient="auto-start-reverse" markerUnits="userSpaceOnUse"><path className="marker-hollow" d="M0 9 L9 0 L18 9 L9 18 Z" /></marker>
    <marker id="uml-inheritance-hollow" markerWidth="19" markerHeight="19" refX="18" refY="9.5" orient="auto" markerUnits="userSpaceOnUse"><path className="marker-hollow" d="M0 0 L18 9.5 L0 19 Z" /></marker>
    <marker id="uml-dependency-open" markerWidth="16" markerHeight="16" refX="15" refY="8" orient="auto" markerUnits="userSpaceOnUse"><path className="marker-open" d="M0 0 L15 8 L0 16" fill="none" stroke="context-stroke" /></marker>
    ...
    const selfLoop = String(relation.source_id) === String(relation.target_id);
    const dx = x2 - x1, dy = y2 - y1, length = Math.hypot(dx, dy) || 1;
    const nx = -dy / length, ny = dx / length;
    const cx = (x1 + x2) / 2 + nx * offset, cy = (y1 + y2) / 2 + ny * offset;
    const geometry = selfLoop ? selfLoopGeometry(sourceBox, offset) : null;
    const sourceEdge = geometry?.sourceEdge || intersectRectangle(sourceBox.center, { x: cx - x1, y: cy - y1 }, sourceBox);
    const targetEdge = geometry?.targetEdge || intersectRectangle(targetBox.center, { x: cx - x2, y: cy - y2 }, targetBox);
    ...
    const labelPoint = geometry?.labelPoint || pointOnQuadratic(sourceEdge, { x: cx, y: cy }, targetEdge);
    const labelX = geometry ? labelPoint.x : labelPoint.x + nx * 12, labelY = geometry ? labelPoint.y : labelPoint.y + ny * 12;
    const sourceTangent = geometry?.sourceTangent || unitVector({ x: cx - sourceEdge.x, y: cy - sourceEdge.y }, { x: ux, y: uy });
    const targetTangent = geometry?.targetTangent || unitVector({ x: targetEdge.x - cx, y: targetEdge.y - cy }, { x: ux, y: uy });
    ...
    return <g key={relation.id} className={className} onMouseEnter={() => onHover(relationContext(relation))} onMouseLeave={() => onHover(null)} onClick={() => onSelect(relation.id)}>
      <path className="relation-hit" d={geometry?.path || `M ${sourceEdge.x} ${sourceEdge.y} Q ${cx} ${cy} ${targetEdge.x} ${targetEdge.y}`} />
      <path className="relation-line" d={geometry?.path || `M ${sourceEdge.x} ${sourceEdge.y} Q ${cx} ${cy} ${targetEdge.x} ${targetEdge.y}`} markerStart={markerFor(relation, "source")} markerEnd={markerFor(relation, "target")} />
