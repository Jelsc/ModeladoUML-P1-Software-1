...
def _type(value: object, default: str = "String") -> str:
    return {
        ...
        "double": "Double", "boolean": "Boolean", "bool": "Boolean", "date": "LocalDate",
        "datetime": "LocalDateTime", "string": "String", "void": "void",
...
def _join_table_name(left, right):
    return "_".join(sorted((_table_name(left), _table_name(right))))
...
def _entity(item, diagram):
    ...
    for attribute in item["attributes"]:
        ...
        scalar_fields.append((field, _type(attribute.get("type"))))
    for relation in diagram["relations"]:
        ...
        if kind == "inheritance":
            if source == item["name"]:
                ...
                inheritance = True
            continue
        role = _field_name(relation.get("label"), other[0].lower() + other[1:])
        if source != item["name"]:
            role = _field_name(relation.get("label"), item["generated_name"][0].lower() + item["generated_name"][1:])
        role = _unique_name(role, used_fields)
        join_column = _field_name(relation.get("label"), f"{_table_name(other)}_id")
        if kind == "association":
            fields.append((role, f"List<{other}>", "many_to_many", join_column, other))
        elif kind in {"composition", "aggregation"}:
            fields.append((role, other, "many_to_one", join_column, other))
        elif kind == "dependency":
            fields.append((role, other, "many_to_one", join_column, other))
    lines = [f"package {PACKAGE}.models;", "", "import com.fasterxml.jackson.annotation.JsonProperty;", "import jakarta.persistence.*;", "import java.time.*;", "import java.util.*;", "", "@Entity"]
    if inheritance:
    ...
    for role, target, relation_kind, join_column, target_class in fields:
        if relation_kind == "many_to_many":
            lines += ["    @ManyToMany", f"    @JoinTable(name = \"{_join_table_name(name, target_class)}\")"]
        else:
            lines += ["    @ManyToOne", f"    @JoinColumn(name = \"{join_column}\")"]
