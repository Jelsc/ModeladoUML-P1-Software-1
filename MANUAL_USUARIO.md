...
El razonamiento es local y no existe fallback a nube: la aplicación no usa API de LLM ni API de voz cloud. El modelo incluido es `mobile/assets/models/assistant.gguf`, una cuantización real `SmolLM2-135M-Instruct-Q4_K_M` de [bartowski en Hugging Face](https://huggingface.co/bartowski/SmolLM2-135M-Instruct-GGUF), basada en SmolLM2 y con licencia Apache-2.0. Es de aproximadamente 105 MB: ocupa menos y arranca mejor en un Snapdragon 720G que Qwen2.5-1.5B, a cambio de menor calidad de instrucciones. Al iniciar, Android lo copia automáticamente por streaming a `files/local-models/assistant.gguf`, usando un archivo temporal y renombrado atómico; el primer inicio necesita aproximadamente 105 MB libres adicionales. No se descarga de Internet durante la ejecución.

El código usa `llamadart 0.8.23` para la inferencia GGUF real en el teléfono. Si el asset no está incluido o no puede copiarse, Settings muestra el estado y un error claro; no se simula inferencia. El contexto está limitado a 1024 tokens y la salida a 32-256 tokens. Q4 usa CPU y puede ser lento o calentar un Snapdragon 720G. La selección manual de otro `.gguf` queda disponible solo como alternativa avanzada.

La voz offline depende de los servicios instalados en cada teléfono Android; se informa esa limitación y el campo de texto continúa disponible. La orquestación agrega manifiesto, esquema y herramientas UAP al prompt y acepta únicamente `{"answer":"..."}` o `{"tool":"TOOL_ID","input":{...}}`. Rechaza JSON mal formado, herramientas desconocidas y campos extra; nunca admite URL arbitrarias, SQL, shell o reflexión. Las herramientas de escritura exigen confirmación antes de `UapClient.invoke`.
...
El teléfono guarda registros genéricos en SQLite local, nunca en Redis. Cada alta, edición o baja se aplica primero a SQLite y queda en una outbox durable con `operationId` UUID, orden de creación y reintentos. Un ejemplo de 15 registros `Member` puede seguir siendo consultado y editado sin backend; al volver la conectividad, usá **Sync now** desde Configuración. La interfaz distingue `Local-only`, `Pending`, `Synced`, `Conflict` y `Failed`.

El backend generado es la autoridad. `GET /uap/v1/sync/changes?since=<cursor>` descarga cambios y `POST /uap/v1/sync/push` recibe hasta 50 operaciones con `operationId`, `entity`, `operation`, `recordId`, `payload` y `baseVersion`. Los duplicados son idempotentes. Si la versión base quedó vieja, el estado es `conflict` y no se pisa silenciosamente el dato del servidor. Si el backend no tiene esos endpoints, la app muestra **Sync protocol unavailable** y las invocaciones UAP online siguen funcionando.
